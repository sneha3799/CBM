...
from config import N_ATTRIBUTES, N_CLASSES
...
def inference(img_path, model, use_relu, use_sigmoid, is_train, resol=299, layer_idx=None):
    ...
    img_6ch = torch.cat([img_tensor, inverted_img], dim=1)  # Shape: [6, H, W]
    input_var = torch.autograd.Variable(img_6ch).cuda()
    print(f'input var shape: {input_var.shape}')
