...
def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        if attr_criterion is None:
            ...
            attr_labels_var = attr_labels.float().cuda(non_blocking=True) if torch.cuda.is_available() else attr_labels.float()

        inputs_var = inputs.cuda(non_blocking=True) if torch.cuda.is_available() else inputs
        labels_var = labels.cuda(non_blocking=True) if torch.cuda.is_available() else labels
        ...
        with torch.set_grad_enabled(is_training):
            if is_training and args.use_aux:
                ...
                if not args.bottleneck: #loss main is for the main task label (always the first output)
                    # print(f'debugging : {outputs.shape}, {labels_var.shape}, {aux_outputs.shape}' )
                    print(f"the outputs are - {outputs}")
                    loss_main = 1.0 * criterion(outputs, labels_var) + 0.4 * criterion(aux_outputs, labels_var)
                    losses.append(loss_main)
                    out_start = 1
                if attr_criterion is not None and args.attr_loss_weight > 0: #X -> A, cotraining, end2end
                    for i in range(len(attr_criterion)):
                        print(f'Debug : {outputs.shape}, {attr_labels_var.shape}')
                        losses.append(args.attr_loss_weight * (1.0 * attr_criterion[i](outputs[i+out_start].squeeze().float(), attr_labels_var[:, i]) \
                ...
                if attr_criterion is not None and args.attr_loss_weight > 0: #X -> A, cotraining, end2end
                    ...
                    # If outputs is a dict, gather values correctly
                    if isinstance(outputs, dict):
                        # Sort by key to ensure consistent order
                        output_tensors = [v for k, v in sorted(outputs.items()) if k >= out_start]
                    else:
                        output_tensors = outputs[out_start:]

                    pred_attr = torch.cat(output_tensors, dim=1)

                    # pred_attr = torch.cat(outputs[out_start:], dim=1)
                    if args.weighted_loss == 'multiple':
                        attr_losses = []
                        for i in range(args.n_attributes):
                            l = args.attr_loss_weight * attr_criterion[i](
                                pred_attr[:, i], attr_labels_var[:, i]
                            )
                            attr_losses.append(l)
                        attr_loss = torch.stack(attr_losses).mean()
                    else:
                        attr_loss = args.attr_loss_weight * F.binary_cross_entropy_with_logits(
                            pred_attr, attr_labels_var
                        )
                    losses.append(attr_loss)

        if args.bottleneck: #attribute accuracy
            if isinstance(outputs, dict):
                # outputs[1] = attribute predictions
                sigmoid_outputs = torch.nn.Sigmoid()(outputs[1])
            else:
                # fallback if model returns a list/tuple instead
            ...
            acc = accuracy(outputs[0].to("cpu"), labels.to("cpu"), topk=(1,)) #only care about class prediction accuracy
            acc_meter.update(acc[0], inputs.size(0))
...
# def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
#     """
#     For networks with both class and attribute supervision (X -> A, Joint CBM, etc.)
#     Tracks class loss, attribute loss, total loss, class accuracy, and concept accuracy.
#     """
#     if is_training:
#         model.train()
#     else:
...
#     concept_acc_meter = AverageMeter()
...
#             acc = binary_accuracy(sigmoid_outputs, attr_labels)
#         else:
#             acc = accuracy(outputs[0].cpu(), labels.cpu(), topk=(1,))
#         acc_meter.update(acc[0], inputs.size(0))

#         # ---- Concept accuracy ----
#         if attr_criterion is not None and not args.bottleneck:
#             concept_logits = torch.cat(outputs[1:], dim=1)
#             concept_probs = torch.sigmoid(concept_logits)
#             concept_preds = (concept_probs > 0.5).float()
#             correct = (concept_preds == attr_labels_var).float()
#             concept_acc = correct.mean().item()
#             concept_acc_meter.update(concept_acc, inputs.size(0))
#         else:
#             concept_acc = 0.0

#         # ---- Separate losses ----
#         class_loss = losses[0] if len(losses) > 0 else torch.tensor(0.0, device=device)
#         attr_loss = losses[-1] if len(losses) > 1 else torch.tensor(0.0, device=device)
#         total_loss = class_loss + attr_loss
...
#         if batch_idx % 50 == 0:
#             print(f"[{'Train' if is_training else 'Val'}] Batch {batch_idx:03d}: "
#                   f"Total={total_loss.item():.4f} | Class={class_loss.item():.4f} | "
#                   f"Attr={attr_loss.item():.4f} | ConceptAcc={concept_acc:.3f}")

#     return total_loss_meter, class_loss_meter, attr_loss_meter, acc_meter, concept_acc_meter
...
def train(model, args):
    ...
    # model = model.cuda()
    # label_smoothing=0.1 # Label smoothing prevents the model from becoming too confident on the training data.
    ...
    if args.use_attr and not args.no_img:
        ...
        if args.weighted_loss:
            ...
            for ratio in imbalance:
                attr_criterion.append(torch.nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor([ratio]).cuda()))
        else:
            for i in range(args.n_attributes):
    ...
    if args.optimizer == 'Adam':
        ...
        #     {'params': model.first_model.resnet.parameters(), 'lr': 1e-5},
        #     {'params': model.first_model.concept_head.parameters(), 'lr': 1e-3},
        #     {'params': model.sec_model.parameters(), 'lr': 1e-3},
    ...
    for epoch in range(0, args.epochs):
        ...
        if args.no_img:
            ...
            # train_total_loss, train_class_loss, train_attr_loss, train_acc, train_concept_acc = run_epoch(
            #     model, optimizer, train_loader, AverageMeter(), AverageMeter(), criterion, attr_criterion, args, is_training=True
            # )
        ...
        if not args.ckpt: # evaluate on val set
            ...
            with torch.no_grad():
                if args.no_img:
                    ...
                    # val_total_loss, val_class_loss, val_attr_loss, val_acc, val_concept_acc = run_epoch(
                    #     model, optimizer, val_loader, AverageMeter(), AverageMeter(), criterion, attr_criterion, args, is_training=False
                    # )
            ...
            # with torch.no_grad():
            #     val_total_loss, val_class_loss, val_attr_loss, val_acc, val_concept_acc = run_epoch(
            #         model, optimizer, train_loader, AverageMeter(), AverageMeter(), criterion, attr_criterion, args, is_training=False
            #     )
            #     val_acc_meter.update(val_acc.avg)
        ...
        #     logger.write(
        #         f"Epoch [{epoch}]: "
        #         f"Train: loss={float(train_total_loss.avg):.4f}, "
        #         f"class_loss={float(train_class_loss.avg):.4f}, "
        #         f"attr_loss={float(train_attr_loss.avg):.4f}, "
        #         f"acc={float(train_acc.avg):.4f}, "
        #         f"concept_acc={float(train_concept_acc.avg):.4f} | "
        #         f"Val: loss={float(val_total_loss.avg):.4f}, "
        #         f"acc={float(val_acc.avg):.4f}, "
        #         f"concept_acc={float(val_concept_acc.avg):.4f}\n"
        #     )
        # else:
        #     logger.write(
        #         f"Epoch [{epoch}]: "
