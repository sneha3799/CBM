...
class BcosResNet(nn.Module):
    def __init__(
        ...
        if logit_bias is not None:
            ...
            self.logit_bias = math.log(1 / (self.num_classes - 1))
        else:
            # Handle case where num_classes == 1 (feature extractor mode)
            self.logit_bias = 0.0
...
def _resnet(
    ...
    **kwargs: Any,
    ...
    if pretrained:
        ...
        # Filter out weights for mismatched layers (like 'fc.weight' and 'fc.bias')
        filtered_dict = {k: v for k, v in state_dict.items() if k in model_dict and v.shape == model_dict[k].shape}

        # Update model dict and load safely
