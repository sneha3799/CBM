...
def gaussian_blur(tensor, kernel_size=5, sigma=1.0):
    ...
    g = g / g.sum()
    g = g.to(tensor.device).unsqueeze(0).unsqueeze(0)
    # Apply depthwise convolution
    return F.conv2d(tensor.unsqueeze(0), g, padding=kernel_size//2).squeeze(0)
...
def grad_to_img(img, linear_mapping, smooth=15, alpha_percentile=99.5):
    ...
    if smooth:
        ...
        alpha = gaussian_blur(alpha, kernel_size=smooth, sigma=smooth/5)
    alpha = to_numpy(alpha)
    alpha = (alpha / np.percentile(alpha, alpha_percentile)).clip(0, 1)
    rgb_grad = rgb_grad * alpha
