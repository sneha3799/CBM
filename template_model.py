...
__all__ = ['MLP', 'Inception3', 'inception_v3', 'End2EndModel', 'ResNet18_Custom', 'BcosResNet18_Custom']
...
class End2EndModel(nn.Module):
    def __init__(self, model1, model2, use_relu=False, use_sigmoid=False, n_class_attr=2):
        ...
        self.first_model = model1   # ResNet → predicts concepts
        self.sec_model = model2     # MLP → predicts class labels
        self.use_relu = use_relu
        self.use_sigmoid = use_sigmoid

    def forward_stage2(self, stage1_out):
        # Apply activation (if needed) to concept predictions
        if self.use_relu:
            attr_outputs = [nn.ReLU()(stage1_out)]
        elif self.use_sigmoid:
            attr_outputs = [torch.sigmoid(stage1_out)]
        else:
            attr_outputs = [stage1_out]

        # Flatten concept predictions for the classifier
        # print('+++', attr_outputs)
        stage2_inputs = torch.cat(attr_outputs, dim=1)
        class_out = self.sec_model(stage2_inputs)

        # Return both class and concept outputs
        return class_out, stage1_out

    def forward(self, x):
        concept_preds = self.first_model(x)
        # print('---', concept_preds)
        class_out, concept_out = self.forward_stage2(concept_preds['concepts'])
        return class_out, concept_out
